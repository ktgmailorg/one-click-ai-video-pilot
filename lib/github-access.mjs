...
export async function repositoryArchiveLocation({ ritEmail, signal }) {
  const token = await repositoryAdministrationToken({ signal });
  const repository =
    process.env.GITHUB_REPOSITORY || "ktgmailorg/video-generator-rit";
  const response = await fetch(
    `https://api.github.com/repos/${repository}/zipball/main`,
    {
      headers: githubHeaders(token),
      redirect: "manual",
      signal,
  ...
  if (response.status < 300 || response.status >= 400 || !location) {
    ...
    throw new Error(
      errorBody.message ||
        `GitHub repository archive failed with HTTP ${response.status}`,
    );
  }
  console.log(
    JSON.stringify({
      event: "rit-repository-download",
      ritIdentity: identityDigest(ritEmail),
      repository,
    }),
  );
